...
public class ViewPathEvaluator implements TypeEvaluator<ViewPoint> {
    ...
    public ViewPoint evaluate(float t, ViewPoint startValue, ViewPoint endValue) {
        ...
        if (endValue.operation == ViewPath2.LINE) {

            //起点判断：
            startX = (startValue.operation == ViewPath2.QUAD) ? startValue.x1 : startValue.x;

            startX = (startValue.operation == ViewPath2.CURVE) ? startValue.x2 : startX;

            startY = (startValue.operation == ViewPath2.QUAD) ? startValue.y1 : startValue.y;

            startY = (startValue.operation == ViewPath2.CURVE) ? startValue.y2 : startY;
        ...
        } else if (endValue.operation == ViewPath2.CURVE) {
        ...
        } else if (endValue.operation == ViewPath2.MOVE) {

            x = endValue.x;
            y = endValue.y;


        } else if (endValue.operation == ViewPath2.QUAD) {

            //起点判断：
            startX = (startValue.operation == ViewPath2.CURVE) ? startValue.x2 : startValue.x;
            startY = (startValue.operation == ViewPath2.CURVE) ? startValue.y2 : startValue.y;
