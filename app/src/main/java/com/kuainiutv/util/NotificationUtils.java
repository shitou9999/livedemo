...
public class NotificationUtils {
    ...
    private void setCompatBuilder(PendingIntent pendingIntent, int smallIcon, String ticker,
        ...
        setCompatBuilder(pendingIntent, R.mipmap.ic_launcher, smallIcon, ticker, title, content, sound, vibrate, lights);
    }
     ...
     * @param ticker
     */
    private void setCompatBuilder(PendingIntent pendingIntent, int largeIcon, int smallIcon, String ticker,
                                  String title, String content, boolean sound, boolean vibrate, boolean lights) {
...
//                requestCode, intent, FLAG);
    ...
    public void notify_progress(PendingIntent pendingIntent, int smallIcon,
                                String ticker, String title, String content, int percent) {

//        setCompatBuilder(pendingIntent, smallIcon, ticker, title, content, sound, vibrate, lights);
        // 进度满了后，设置提示信息
        if (percent == 100) {//下载完成后发送通知并有通知声音。
            setCompatBuilder(pendingIntent, smallIcon, ticker, title, content, true, true, true);
            cBuilder.setContentText("下载完成").setProgress(0, 0, false);
        } else {//因为进度条要实时更新通知栏也就说要不断的发送新的提示，所以这里不开启通知声音。
            setCompatBuilder(pendingIntent, smallIcon, ticker, title, content, false, false, false);
        }

        cBuilder.setProgress(100, percent, false);
//        cBuilder.notify();
        sent();
    }
